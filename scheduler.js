...
const getToken = async () => {
  try {
    ...
    if (response.data.success) {
      ...
      throw new Error('Error getting authentication token: ' + response.data.error);
    }
  } catch (error) {
...
const chargeCardOnFile = async (authToken, data, attempt = 1) => {
  try {
    ...
    if (response.data.status == 'Success') {
      ...
      console.log(data.externalReference, response.data.data.paybossRef, data.data.amount);
      // queryTransactionStatus(data.externalReference, response.data.data.paybossRef, data.data.amount);

      if (attempt <= 3) { // Limiting retries to 3 attempts
...
const checkEntriesAndCharge = async () => {
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      const collectionDate = typeof data.collectionDate === 'string' ? data.collectionDate : null;
    ...
    const { data: { authToken } } = await getToken();
    ...
    console.log('Charging completed for today\'s entries within the date range:', todayStr);
  } catch (error) {
...
const queryTransactionStatus = async (externalReference, paybossRef, amountToCollect) => {
  try {
      // Get authentication token
      const { data: { authToken } } = await getToken();
      ...
      if (response.data.data && response.data.data.status === 'Complete') {
          ...
          const { totalAmount, paybossRef, customerName, correlationReference } = response.data.data;
          ...
          if (correlationReference && paybossRef && totalAmount) {
              try {
                  ...
                  const querySnapshot = await clientsRef.where('externalReference', '==', correlationReference).get();

                  // const UdatingEntries = [];
                  querySnapshot.forEach(async (doc) => {
                      try {
                          const querySnapshotData = doc.data(); 
                          const repaymentSummaryRef = doc.ref.collection('repaymentSummary');
                          const latestRepaymentQuery = await repaymentSummaryRef.orderBy('timeStamp', 'desc').limit(1).get();

                          latestRepaymentQuery.forEach(async (latestRepaymentDoc) => {
                            try {
                                const latestRepaymentData = latestRepaymentDoc.data();
                                // Now you can access the data of the document
                                const expectedAmount = latestRepaymentData.expectedAmount;
                                ...
                                if (amountToCollect == expectedAmount) {
                                  ...
                                  let newLastDate = lastDate.toISOString().split('T')[0];
                                  ...
                                  const formattedNewCollectionDate = newCollectionDate.toISOString().split('T')[0];
                                  const pendingAmount = Math.max(expectedAmount - amountToCollect, 0);
                                  ...
                                  await repaymentSummaryRef.add({
                                ...
                                } else if (amountToCollect !== expectedAmount && expectedAmount > 0) {
                                    ...
                                    let collectedDate = newCollectionDate.toISOString().split('T')[0];
                                    // iterations++; 
          ...
          console.error('Error querying transaction status: ' + response.data.message);
      }
  } catch (error) {
